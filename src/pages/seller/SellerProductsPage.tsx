...
import { createProduct, deleteProduct, fetchAllProducts, updateProduct } from '../../lib/api';
...
export default function SellerProductsPage() {
  ...
  const load = useCallback(async () => { setLoading(true); try { setProducts(await fetchAllProducts()); } finally { setLoading(false); } }, []);
  useEffect(() => { load(); }, [load]);
  ...
  const save = async () => { if (!form.name.trim() || !form.price) { setError('Name and price are required.'); return; } setSaving(true); setError(''); try { const uploadedUrls: string[] = []; for (const pending of pendingFiles) { const result = await uploadFile(pending.file); uploadedUrls.push(result.url); URL.revokeObjectURL(pending.previewUrl); } const payload = { name: form.name.trim(), brand: form.brand || null, category: form.category, price: Number(form.price), description: form.description || null, images: [...form.images, ...uploadedUrls], sizes: form.sizes, colors: form.colors, in_stock: form.in_stock, stock_quantity: Number(form.stock_quantity) || 0, is_featured: form.is_featured, tags: form.tags }; if (editingId) await updateProduct(editingId, payload); else await createProduct(payload); invalidateCache(); setShowModal(false); setPendingFiles([]); await load(); } catch (reason) { setError(reason instanceof Error ? reason.message : 'Product could not be saved.'); } finally { setSaving(false); } };
  ...
  const removeProduct = async (product: Product) => { if (!window.confirm(`Delete ${product.name}?`)) return; for (const image of productImages(product)) { try { const key = image.split('/').pop()?.split('.')[0]; if (key) await deleteFile(key); } catch { /* retain DB deletion even when storage cleanup fails */ } } await deleteProduct(product.id); invalidateCache(); await load(); };
