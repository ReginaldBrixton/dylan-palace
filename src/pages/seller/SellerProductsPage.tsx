...
export default function SellerProductsPage() {
  ...
  const save = async () => {
    ...
    try {
      ...
      await saveAdminProduct(editingId, { name: form.name.trim(), brand: form.brand || null, category: form.category, price: Number(form.price), description: form.description || null, images: [...form.images, ...uploadedUrls], sizes: form.sizes, colors: form.colors, in_stock: form.in_stock, stock_quantity: Number(form.stock_quantity) || 0, is_featured: form.is_featured, tags: form.tags });
  ...
  const moveExisting = (index: number, direction: -1 | 1) => setForm((current) => { const target = index + direction; if (target < 0 || target >= current.images.length) return current; const images = [...current.images]; [images[index], images[target]] = [images[target], images[index]]; return { ...current, images }; });
  const movePending = (index: number, direction: -1 | 1) => setPendingFiles((current) => { const target = index + direction; if (target < 0 || target >= current.length) return current; const next = [...current]; [next[index], next[target]] = [next[target], next[index]]; return next; });
  ...
  return <AdminShell title="Products" description={`${products.length} products across the current catalogue.`} actions={action}><label className="relative mb-5 block max-w-xl"><span className="sr-only">Search products</span><Search size={17} className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--color-muted)]"/><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search name, brand or category" className="h-12 w-full border border-[var(--color-border)] bg-white pl-11 pr-4 text-sm outline-none focus:border-[var(--color-ink)]"/></label>{loading ? <div className="grid grid-cols-2 gap-4 md:grid-cols-3 xl:grid-cols-4">{Array.from({ length: 8 }).map((_, index) => <div key={index} className="aspect-[4/5] animate-pulse bg-white"/>)}</div> : <div className="grid grid-cols-2 gap-4 md:grid-cols-3 xl:grid-cols-4">{filtered.map((product, index) => <ProductCard key={product.id} product={product} index={index} onEdit={openEdit} onDelete={removeProduct}/>)}</div>}<AnimatePresence>{showModal ? <ProductFormModal editingId={editingId} form={form} setForm={setForm} error={error} saving={saving} aiLoading={aiLoading} pendingFiles={pendingFiles} onClose={() => setShowModal(false)} onSave={save} onFileUpload={upload} onAIAnalyze={analyze} onRemoveImage={removeImage} onMoveExisting={moveExisting} onMovePending={movePending}/> : null}</AnimatePresence></AdminShell>;
}
